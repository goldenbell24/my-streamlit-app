...
def decide_genre(answers: dict) -> tuple[str, str]:
    ...
    top.sort(key=lambda x: list(GENRES.keys()).index(x))  # 고정 우선순위
    chosen = top[0]

    explanation = f"답변을 종합해보면 **{chosen}** 장르가 특히 잘 어울려요!"
...
def make_reason(chosen_genre: str, answers: dict, movie: dict) -> str:
    ...
    if q3 == "감동 스토리":
        reason = f"스토리 몰입을 좋아하는 당신에게 **{title}**가 잘 맞아요."
    elif q3 == "시각적 영상미":
        reason = f"영상미를 중요하게 생각한다면 **{title}** 같은 작품이 만족도가 높아요."
    elif q3 == "깊은 메시지":
        reason = f"여운/메시지를 선호하는 당신에게 **{title}**는 생각할 거리를 줄 수 있어요."
    else:  # 웃는 재미
        reason = f"가볍게 웃고 싶을 때 **{title}**는 기분 전환용으로 좋아요."

    if q4 == "힐링" and chosen_genre in ["로맨스", "드라마"]:
        reason += " 편안하게 힐링하며 보기 좋아요."
    elif q4 == "액티비티" and chosen_genre in ["액션", "SF"]:
        reason += " 템포가 살아있어 스트레스 해소에 딱이에요."

    if isinstance(rating, (int, float)) and rating >= 7.5:
        reason += " 평점도 높은 편이라 실패 확률이 낮아요."
...
st.write("간단한 질문 5개로 당신의 성향을 알아보고, 어울리는 장르의 인기 영화를 추천해드려요! 🍿")
st.divider()
...
if st.button("결과 보기", type="primary"):
    ...
    with st.spinner("분석 중..."):
        try:
            ...
            st.error("네트워크 문제로 TMDB API 호출에 실패했어요.")
            st.caption(f"에러 상세: {e}")
            st.stop()
    ...
    st.subheader("🎞️ 인기 영화 추천 TOP 5 (격자 보기)")

    if not movies:
        st.info("해당 장르에서 추천할 영화를 찾지 못했어요.")
        st.stop()

    # ----------------------------
    # ✅ 그리드 출력 (핵심 변경)
    # ----------------------------
    n_cols = 3  # 2로 바꾸면 더 큼직하게 보임
    ...
    for i, m in enumerate(movies):
        ...
        with col:
            with st.container(border=True):
                ...
                st.markdown(f"**{title}**")
                st.caption(f"⭐ 평점: {rating:.1f}" if isinstance(rating, (int, float)) else "⭐ 평점 정보 없음")

                with st.expander("줄거리 / 추천 이유 보기"):
                    st.markdown("**줄거리**")
                    st.write(overview)
                    st.markdown("**이 영화를 추천하는 이유**")
                    st.write(make_reason(chosen_genre, answers, m))
